...
async def generate_and_rm(
    ...
    # Training may score a complete prompt group atomically.  Evaluation is
    # still per-sample, even when training uses a group-aware RM.
...
async def generate_and_rm_group(
    ...
    if not state.aborted and args.group_rm and not evaluation:
...
async def abort(args: Namespace, rollout_id: int) -> list[list[Sample]]:
    ...
    # Make sure locally submitted tasks finish cancellation.  A remote reward
    # service can otherwise make this cleanup wait forever after its output is
    # no longer useful.  The rollout caller cancels surplus tasks before this
    # function is entered; retain a finite grace period for their HTTP/SGLang
    # cleanup and then abandon any stragglers.
    timeout_seconds = float(getattr(args, "rollout_abort_drain_timeout", 30.0))
    if timeout_seconds < 0:
        raise ValueError("rollout_abort_drain_timeout must be non-negative")
    ...
    while state.pendings:
        ...
        if remaining <= 0:
            for task in state.pendings:
                task.cancel()
            logger.warning("Stopped waiting for %d aborted rollout task(s) after %.1fs", len(state.pendings), timeout_seconds)
            break
        done, state.pendings = await asyncio.wait(
            state.pendings, timeout=remaining, return_when=asyncio.FIRST_COMPLETED,
        )
        if not done:
            continue
...
async def generate_rollout_async(
    ...
    # Custom reward functions can use this stable ID for rollout-level policy,
    # such as selecting one shared adversary temperature for a GRPO batch.
...
async def eval_rollout(args: Namespace, rollout_id: int) -> tuple[dict[str, dict[str, list[Any]]], list[list[Sample]]]:
    # Group-aware reward functions require a complete training prompt group.
    # Evaluation calls them once per sample (see ``generate_and_rm``), which
    # lets the same custom RM report scalar validation rewards.
    args.current_rollout_id = rollout_id
    results = {}
